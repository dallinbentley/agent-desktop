...
The daemon SHALL listen on ~/.agent-computer/daemon.sock, accepting newline-delimited JSON requests and returning JSON responses. Each request/response SHALL have an id field for correlation and responses SHALL include timing.elapsed_ms.
...
- **THEN** CLI prints "Starting agent-computer daemon..." and spawns daemon
