...
- **WHEN** `agent-desktop snapshot -i --app "System Settings"`
- **THEN** same output format as Swift version

### Requirement: New open --with-cdp flag
The CLI SHALL support `agent-desktop open --with-cdp <app>` which relaunches the target app with --remote-debugging-port. It SHALL assign a deterministic port and track it in daemon state.
...
#### Scenario: JSON mode
- **WHEN** `agent-desktop snapshot -i --json`
...
- **THEN** error includes suggestion: "Run `agent-desktop open --with-cdp <app>` to enable rich UI interaction."
