...
- **WHEN** user runs `agent-desktop open "Safari"` and Safari is running
- **THEN** system activates Safari, bringing it to the front

#### Scenario: Launch app not running
- **WHEN** user runs `agent-desktop open "TextEdit"` and TextEdit is not running
- **THEN** system launches TextEdit and waits for it to become active

#### Scenario: App not found
- **WHEN** user runs `agent-desktop open "Safarri"` (typo)
...
- **WHEN** user runs `agent-desktop status` with Finder in front
...
- **WHEN** user runs `agent-desktop get apps`
