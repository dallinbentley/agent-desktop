...
- **WHEN** `agent-computer click @e3 --app "System Settings"` and System Settings is in background
...
- **WHEN** `agent-computer click @e7 --app Spotify` and Spotify is in background
...
- **WHEN** `agent-computer screenshot --app Spotify` and Spotify is behind other windows
...
- **WHEN** `agent-computer click 500 300 --app Spotify --foreground`
- **THEN** Spotify is brought to front, then CGEvent click at (500, 300)

#### Scenario: Coordinate click without foreground
- **WHEN** `agent-computer click 500 300 --app Spotify` without --foreground
...
- **WHEN** `agent-computer snapshot -i` with Finder frontmost
- **THEN** returns Finder's AX tree without changing focus

#### Scenario: Snapshot background app
- **WHEN** `agent-computer snapshot -i --app Spotify` with Terminal frontmost
